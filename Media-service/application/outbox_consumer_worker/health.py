...
app = FastAPI(
    title="Outbox Consumer Worker Health Check",
    version="1.0.0",
)

setup_dishka(outbox_consumer_container, app)
...
async def check_rabbitmq(consumer: RabbitMQConsumer) -> bool:
    try:
...
async def ready_check(
    ...
    consumer: FromDishka[RabbitMQConsumer],
) -> dict[str, Any]:
    database_ok = await check_database(session)
    rabbitmq_ok = await check_rabbitmq(consumer)
